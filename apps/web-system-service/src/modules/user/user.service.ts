...
export class UserService extends Logger {
    constructor(
        ...
        private readonly redisService: RedisService // private readonly databaseService: DatabaseService
    ) {
        super()
    }
    ...
    public async httpCommonCodexUser(response: Response) {
        return await plugin.fetchCommonCodexer({ width: 120, height: 40 }).then(async ({ sid, text, data }) => {
            ...
            return await this.redisService.setStore({ key, data, seconds: 300 }).then(async ({ seconds }) => {
                this.logger.info('httpCommonCodexUser', {
                    log: { message: '图形验证码发送成功', seconds, key, text }
                })
